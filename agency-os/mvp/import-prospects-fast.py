    ...
    for st, entries in by_state.items():
        for i in range(len(entries)):
            for j in range(i + 1, len(entries)):
                ...
                # exact pruning: real_quick_ratio >= quick_ratio >= ratio, so skipping
                # pairs whose upper bound is below threshold changes nothing in output
                if not contained and (sm.real_quick_ratio() < NEAR_DUPE_RATIO or sm.quick_ratio() < NEAR_DUPE_RATIO):
                    continue
                ratio = sm.ratio() if not contained or True else 0.0
