...
- **2026-08-04, moodboards reject faces — own-photos-first has a limit.** Engineering's "seven refs come from Taylor's own pool" premise was WRONG for an industrial/landscape board: the 53-file pool is 28 portraits + ~25 selfie/group/lifestyle frames, and Higgsfield's own board guidance lists faces as a red-X failure case. Own-photos-first still holds for AZ Homes (residential geometry) and all proof-of-work; it has nothing to give a board about grid infrastructure or landscape. Check the pool's *content*, not its size, before claiming it covers a need.
- **2026-08-04, enforce the branding rule at the REFERENCE layer, not just the output.** Legible wordmarks were blurred out of refs before upload (Chase/Wells Fargo/EY on skyline towers, highway guide signs). Feeding legible text to a generative board invites the exact failure the no-branding rule exists to prevent. Blur before upload, every time.
- **2026-08-04, don't pad a board to clear a counter.** Higgsfield's UI wants 20+ refs and flags fewer as "Bad" while building fine at 5. Nineteen strong refs beat twenty with a weak one — a bad reference actively steers generations wrong. Flag the counter, ignore it.
- **2026-08-04, capture technique:** when Chrome blocks bulk downloads, full-size-original screen capture is an acceptable fallback (784–1490px, scored Good) — but render portrait-orientation refs *rotated 90°* to fill the long viewport axis, then rotate back, or they capture at ~520px and tank the board's quality score.
