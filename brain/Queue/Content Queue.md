...
The production line. One `##` section per post, newest on top.

**Statuses**: `idea` → `drafted` → `visuals-planned` → `fact-checked` →
`approved` → `produced` → `posted` · (`rejected: <reason>` at any point)

Only Taylor sets `approved`, and only from `fact-checked`. A `rejected`
reason teaches the agents — be specific.
...
## Rejected (kept for the agents to learn from)

## Monsoon vs. your panels — what actually happens
- **status**: rejected: owner — dirty panels / efficiency trivia isn't a
  real homeowner pain point. Talk about REAL pains and objections (bill
  shock, quote confusion, fear of a bad deal, rising rates), treat them
  lightly, and always land on a soft CTA.

## The 30% federal credit is gone — honest 2026 math
- **status**: rejected: owner redirected editorial focus — expired-credit
  content is myth-busting-only when someone's being actively pitched the
  dead credit, never a pillar. See [[Editorial Direction]].
